...
function filterEntries() {
    ...
    // Zeige eine Info an, wenn keine Ergebnisse gefunden wurden
...
function deleteCard(btn) {
    ...
    if (id) {
        ...
        deleteExhibitInDatabase(id); // see admin.js
    }
    entry.remove();
}
...
function setHTML(html, outputDivName, textId) 
{
    outputDiv = document.getElementById(outputDivName);
    if (outputDiv!=null)
    {
        console.log("setting textid="+textId+";"+outputDivName+".innerHTML="+html);
        outputDiv.innerHTML = html;
        outputDiv.setAttribute("data-textId", textId);
    }
    else
        console.log(outputDivName + " not found");
}
...
function setLCID(lcid)
{
    ...
    if (lcid == null)
        lcid = 1031; //fallback german

    sessionStorage.setItem("lcid", lcid);
    return lcid;
}

function getLCID() {return sessionStorage.getItem("lcid");}